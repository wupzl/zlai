...
public class WebSearchToolHandler implements ToolHandler {
    ...
    public boolean supports(String toolKey) {
        return StringUtils.hasText(toolKey) && "web_search".equalsIgnoreCase(toolKey.trim());
    }

    @Override
    public ToolExecutionResult execute(ToolExecutionRequest request) {
        if (request == null || !supports(request.getToolKey())) {
            return ToolExecutionResult.fail("Unsupported tool");
        }
